...
function ServicesPage() {
  ...
  const [currentService, setCurrentService] = useState({ id: null, nom: '', code_service: '', description: '' });
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      console.error(`Erreur lors de ${isEditing ? 'la modification' : 'la création'}`, err);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white shadow-xl rounded-xl overflow-hidden">
                    ...
                    {service.secretaire_responsable ? (
                      <div className="flex items-center">
                        <div className="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center mr-3">
                          <span className="text-green-600 font-semibold text-sm">{service.secretaire_responsable.prenom.charAt(0)}</span>
                        </div>
                        <div>
                          <div className="text-sm font-medium text-gray-900">{service.secretaire_responsable.prenom} {service.secretaire_responsable.nom}</div>
                          <div className="text-xs text-green-600">Responsable assigné</div>
                        </div>
                      </div>
                    ) : (
                      <div className="flex items-center">
                        <div className="w-8 h-8 bg-orange-100 rounded-full flex items-center justify-center mr-3"><div className="w-2 h-2 bg-orange-500 rounded-full"></div></div>
                        <div>
        ...
        {services.length === 0 && (
          <div className="text-center py-12">
            <div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4"><div className="w-8 h-8 bg-green-600 rounded"></div></div>
            ...
            <button onClick={handleOpenCreateModal} className="bg-green-600 hover:bg-green-700 text-white font-medium py-2 px-4 rounded-lg transition-colors duration-200">Ajouter un Service</button>
          </div>
        )}
      </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={isEditing ? 'Modifier le Service' : 'Ajouter un Nouveau Service'}>
        <form onSubmit={handleFormSubmit} className="space-y-6">
          ...
          <div>
            <label className="block text-gray-700 font-semibold mb-2">Code du service</label>
            <input type="text" value={currentService.code_service} onChange={(e) => setCurrentService({ ...currentService, code_service: e.target.value })} className="w-full border-2 border-gray-200 rounded-lg px-4 py-3 text-gray-700 leading-tight focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-200 transition-all duration-200 font-mono" placeholder="ex: DOM" required />
          </div>
          <div>
            <label className="block text-gray-700 font-semibold mb-2">Description (Optionnel)</label>
            <textarea value={currentService.description} onChange={(e) => setCurrentService({ ...currentService, description: e.target.value })} className="w-full border-2 border-gray-200 rounded-lg px-4 py-3 text-gray-700 leading-tight focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-200 transition-all duration-200 resize-none" rows="3" placeholder="Description détaillée du service et de ses responsabilités..."></textarea>
          </div>
