...
import { Search, Settings, MoreHorizontal, Edit, Trash2, Calendar, Clock, Users, AlertCircle } from 'lucide-react';
...
export function OnCallTable({
  ...
  const services = useMemo(() => 
    Array.from(new Set(periods.map(period => period.service))).sort(),
    [periods]
  );
  ...
  const filteredPeriods = useMemo(() => {
    return periods.filter(period => {
      ...
      const matchesSearch = 
        ...
        period.service.toLowerCase().includes(search) ||
        period.assignedAgents.some(agent => agent.toLowerCase().includes(search));
      
      const matchesService = serviceFilter === "all" || period.service === serviceFilter;
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {filteredPeriods.map((period, index) => {
              ...
              const TypeIcon = typeConfig[period.type]?.icon || Settings;
              return (
                <tr key={period.id} className="border-t hover:bg-gray-50/50 group">
                  ...
                  <td className="p-4"><span className="font-medium bg-gray-100 text-gray-800 px-2 py-1 rounded-full">{period.service}</span></td>
                  ...
                  <td className="p-4 text-right">
                    ...
                    <div className="opacity-0 group-hover:opacity-100 transition-opacity flex justify-end gap-2">
                      <button onClick={() => onEditPeriod(period)} className="p-2 rounded-md hover:bg-gray-200">
                        <Edit className="w-4 h-4 text-blue-600" />
                      </button>
                      <button onClick={() => onDeletePeriod(period.id)} className="p-2 rounded-md hover:bg-gray-200">
